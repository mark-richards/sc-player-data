...
def _compute_effective_avg(sc_df: pd.DataFrame,
                           player_list_df: pd.DataFrame | None = None) -> pd.DataFrame:
    """
    ...
    def _missed_breakdown(row):
        ...
        missed_non_bye = max(0, total_rounds - row["games_played"] - missed_bye)
    ...
    if player_list_df is not None and not player_list_df.empty:
        ...
        if not missing.empty:
            def _zero_breakdown(team):
                bye_rds    = all_rounds - team_played_rounds.get(team, all_rounds)
                missed_bye = len(bye_rds)
                missed_non_bye = total_rounds - missed_bye
                return missed_bye, missed_non_bye

            missing[["missed_bye", "missed_non_bye"]] = missing["team"].apply(
                lambda t: pd.Series(_zero_breakdown(t))
            )
            missing["raw_avg"]      = 0.0
...
def load_draft_board(draft_csv, player_list_csv, alias_map: dict,
    ...
    if sc_df is not None and not sc_df.empty and player_list_csv.exists():
        ...
        player_data    = _compute_effective_avg(sc_df, player_list_df)
